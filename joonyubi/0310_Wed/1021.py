...
for target in targets:
    option2 = queue.index(target)   #2번방식으로 이동 (direction: cw)
    option3 = len(queue) - queue.index(target)  #3번방식으로 이동 (direction: ccw)
    steps = min(option2, option3)

    if steps == option2: direction = 'cw'
    else: direction = 'ccw'

    if direction == 'cw':   #시계방향으로 돌때는 queue의 첫째 값을 빼서 마지막값으로 넣어준다.
        for _ in range(steps):
            temp = queue.pop(0)
            queue.append(temp)
    
    else:                   #반시계방향으로 돌때는 queue의 마지막 값을 빼서 첫째값으로 넣어준다.
        for _ in range(steps):
            temp = queue.pop(-1)
            queue.insert(0,temp)
