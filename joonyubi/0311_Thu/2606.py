...
'''
def dfs(adjacent_graph, cur_node, visited_array):
    visited_array.append(cur_node)
    for adjacent_node in adjacent_graph[cur_node]:
        if adjacent_node not in visited_array:
            dfs(adjacent_graph, adjacent_node, visited_array)
'''

def dfs(adjacent_graph, cur_node, visited_array):       #dfs 함수 설정(인자 : 그래프, 현재노드, 방문이력리스트)
    visited_array.append(cur_node)
    for adjacent_node in adjacent_graph[cur_node]:
        if adjacent_node not in visited_array:
            dfs(adjacent_graph, adjacent_node, visited_array)
